...
export function LatestItem({ article }: { article: Article }) {
  return (
    <Link href={`/article/${article.slug}`} className="card-lift group flex gap-4 border-b border-rule py-4">
      <div className="h-16 w-22 flex-shrink-0 overflow-hidden rounded">
        <div
      ...
      <div className="min-w-0">
        ...
        <h4 className="mb-1.5 font-display text-base font-medium leading-snug transition-colors group-hover:text-signal">
          {article.title}
        </h4>
...
export function MostReadItem({ article, rank }: { article: Article; rank: number }) {
  return (
    <Link href={`/article/${article.slug}`} className="group flex gap-4">
      <span className="font-mono text-3xl font-medium leading-none text-rule transition-colors group-hover:text-signal">
        {String(rank).padStart(2, "0")}
      </span>
      <div>
        <h4 className="mb-1.5 font-display text-[15.5px] font-medium leading-snug transition-colors group-hover:text-signal">
          {article.title}
        </h4>
