...
export default async function HomePage() {
  ...
  const [{ data: latest }, { data: mostRead }, { data: categories }] = await Promise.all([
    supabase
      .from("articles")
  ...
  if (!featured) {
    return (
      <main className="mx-auto max-w-6xl px-6 py-20 text-center sm:px-10">
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10 sm:px-10">
      <div className="mb-10 flex flex-wrap gap-2.5">
        ...
        {cats.map((c) => (
          <Link
            ...
            className="rounded-full border border-rule px-3.5 py-1.5 text-[12.5px] font-medium text-ink-muted hover:border-ink hover:text-ink"
          >
            {c.name}
          </Link>
        ))}
      ...
      {topRead.length > 0 && (
        <section className="mb-14 border-y border-rule py-6">
